    ...
    methods: {
        ...
        getCount() {
            return new Promise(resolve => {
                let url = !this.search_text ? '/product-type/count' : '/product-type/count?keywords=' + this.search_text;
        ...
        getData(page) {
            return new Promise(resolve => {
                let url = '/product-type/list/' + page;
        ...
        delete() {
            if(this.check.length > 0) {
                loading.show = true;
                axios.delete('/product-type/delete', {data: this.check}).then(async res => {
                    if (res.data.status) {
...
let set_info = createApp({
    ...
    methods: {
        ...
        save() {
            let url = this.mode === 'create' ? '/product-type/insert' : '/product-type/update';
            loading.show = true;
